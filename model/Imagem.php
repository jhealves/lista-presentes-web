...
class Imagem {
    ...
    public function uploadImage($email, $arquivo, $pasta) {
        $tipo = $arquivo['type'];
        ...
        $foto = base64_encode($nome) . rand(0,10000) . '.jpg';
        ...
        return 'ok';
        $u = new Usuario();
        if($antigo = $u->getUsuario($email)){
            unlink($pasta . $antigo['foto']);
        }

        $u->addFoto($email, $nome);
        
        
    }

    public function erroTamanho($tamanho) {
